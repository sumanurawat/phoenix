...
class Advanced3DPopulationExplorer:
    ...
    def create_pca_3d_visualization(self):
        ...
        # Get country names
        latest_year = int(self.data['year'].max())
        country_names = [str(name) for name in self.data[self.data['year'] == latest_year]['country_name'].values]
        
        # Create 3D scatter
        fig = go.Figure(data=[go.Scatter3d(
            x=[float(x) for x in pca_result[:, 0]],
            y=[float(y) for y in pca_result[:, 1]],
            z=[float(z) for z in pca_result[:, 2]],
            mode='markers',
            marker=dict(
                size=8,
                color=[float(c) for c in features['population_millions']],
                colorscale='Viridis',
                showscale=True,
    ...
    def create_tsne_3d_visualization(self):
        ...
        # Create 3D scatter
        fig = go.Figure(data=[go.Scatter3d(
            x=[float(x) for x in tsne_result[:, 0]],
            y=[float(y) for y in tsne_result[:, 1]],
            z=[float(z) for z in tsne_result[:, 2]],
            mode='markers',
            marker=dict(
                size=8,
                color=[int(c) for c in clusters],
                colorscale='Rainbow',  # Changed from 'Set3' to valid colorscale
                showscale=True,
...
def main():
    ...
    # Create explorer with explicit data path
    data_path = "/Users/sumanurawat/Documents/GitHub/phoenix/experiments/population/data"
    explorer = Advanced3DPopulationExplorer(data_dir=data_path)
