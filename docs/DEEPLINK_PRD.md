...
2.  **Navigation to "Create New Link":** User navigates to the "Create New Short Link" page (e.g., via their profile page at `/apps/deeplink/profile/converter`).
3.  **Link Creation:**
    *   User enters the original URL. Their email and user ID are known from their session.
    *   System generates a unique short link associated with their account.
    *   The new short URL is displayed.
4.  **Navigation to "My Links" Dashboard:** User accesses the "My Shortened Links" page (e.g., via their profile page at `/apps/deeplink/profile/my-links`).
5.  **Link Management & Analytics Viewing:**
    *   The "My Links" page displays a table of all links created by the user, showing the short URL (full, clickable), original URL, creation date, and total click count.
...
*   **FR1.4: User Interface:**
    *   Clean, intuitive interface for link generation (`/apps/deeplink/profile/converter`).
    *   Clear display of the generated short link with an easy way to copy it.
    *   Dashboard for viewing created links (`/apps/deeplink/profile/my-links`).
...
*   **FR3.5: "My Links" Dashboard:**
    *   A dedicated page (`/apps/deeplink/profile/my-links`) for authenticated users to:
        *   View a list/table of all their created links (short URL, original URL, creation date, total clicks).
        *   Copy short URLs.
        *   Access the page to create new short links.
